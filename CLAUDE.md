...
**Phase 5 QA still owed (user):** the interactive half was NOT verified — the browser automation pane hung, and per BUILD_PLAN the click-through is the user's job anyway. Specifically untested: actually drawing a signature and submitting; the typed-name fallback; the DB row + storage upload; Step 2 flipping to complete in the portal; the "already submitted" state on revisit; "Create a new link" invalidating the old URL; and a real phone pass for signature legibility. **Also unverified: `MIN_SIGNATURE_BYTES = 1200` in `validation.ts` is an untested guess** — measure a real cleared-canvas PNG vs. a real signature during QA and tune it, or a legitimate faint signature could be rejected (or an empty one accepted).
