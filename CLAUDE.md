...
**Phase 8 follow-ups (Nick's QA feedback, 2026-08-18):**

- **Interviewer(s)** is a required multi-select of active staff, not free text. Stored as comma-separated `admin_users` ids in `interviews.interviewers` (so a future "interviews I ran" filter has something to match); the profile resolves ids to names, falling back to the raw value for any legacy free-text entry.
- **All 7 rubric criteria are required** before an interview saves — a half-scored rubric can't support a decision. Notes stay optional, and the clear-blanked-scores branch is gone since every criterion is always written.
- **C2L review has three states**, because "not looked at yet" and "looked, and it's wrong" are different facts: `pending_verification` → **Pending staff review**, `complete` → **Reviewed — verified** (✓), and the new `needs_attention` → **Reviewed — incomplete** (✗). Flagging **requires** a `staff_note` saying what's missing; the student sees it on their step and gets an "I fixed it — check again" button, so a flagged step is recoverable. `needs_attention` is staff-only (absent from `studentAllowedStatuses` and from the RLS WITH CHECK list) but students can move OUT of it by re-reporting, which is the intended loop. Status is conveyed by text as well as colour/glyph.
- **Migration 0008** adds the `needs_attention` enum value and `step_progress.staff_note`. **Must be pasted into the SQL editor** — the JS client can't run DDL, so unlike 0005/0007 this one was NOT applied automatically. The C2L review UI returns an error naming 0008 until it is. Run it on its own: Postgres won't let a new enum value be used in the same transaction that adds it.
- **Bulk actions on the applicant table** (`applicant-table.tsx` is now a client component owning selection): row checkboxes, a header select-all covering everything matching the current filters, an indeterminate state, and **shift-click range selection** (union, not replace) so "first, shift-last" grabs 30 students in two clicks. A sticky action bar offers C2L verify/flag (note required to flag), track A/B, and recording a decision — behind a **two-step confirm**, since one mis-click could otherwise reclassify a cohort.
- **Bulk design choices worth keeping:** each affected application gets its OWN audit entry (a student's history reads the same whether they were touched individually or in a batch, just with `bulk: true` in the payload); rows the action can't apply to are **skipped and counted back to the caller** rather than forced (bulk-verifying an unreported step would invent a submission); bulk track sets `track_overridden`; and **releasing decisions is deliberately NOT bulk-able** — revealing outcomes to a cohort is irreversible and stays on the individual profile.
